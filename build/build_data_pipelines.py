...
for pipeline_type, pipeline_descriptors in list(PIPELINES_PAYLOAD.items()):
    ...
    data_pipelines_infrastructure += pipeline_infrastructure_code_str + "\n\n"

infrastructure_template = open(PLATFORM_INSTANCE_INFRASTRUCTURE_SOURCE_PATH, "r").read()
infrastructure_template = infrastructure_template.replace("{data_pipeline_infrastructure}",
                                                          "{}".format(data_pipelines_infrastructure))
